async function register({
  ...
  registerClientRoute,
    ...
    const currentPending = await peertubeHelpers.translate(
      "Currently pending"
    );
    const minPayout = await peertubeHelpers.translate(
      "Minimum for payout is"
    );
    ...
    const walletLabel = await peertubeHelpers.translate(
      "Your wallet address"
    );
  ...
  registerHook({
    ...
    handler: async (params) => {
      if (params.path.startsWith("/my-account")) {
        ...
        const content =
          `
          <a _ngcontent-dke-c79="" id="monetization-link" routerlinkactive="active" class="title-page title-page-settings ng-star-inserted" href="/p/monetization">
            ${Monetizeation}
          </a>
        `;
        ...
        for (let i = 0; i < data.history.length; i++) {
          ...
          historyHtml += `
            <tr>
              ...
              <td>${history.state === 0 ? pendingTd : processedTd}</td>
            </tr>
          `;
        }
      }
  ...
  // Register the route (see options above)
